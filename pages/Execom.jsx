...
async function fetchData(TeamName) {
  try {
    const TeamSnapshot = await firestore.collection(TeamName).orderBy("id").get();
...
const Execom = ({ TeamsData }) => {
  ...
  return (
    <div className="bg-white flex flex-col justify-start items-center h-full w-full mt-20 mb-10   overflow-hidden" style={{ overflow: 'hidden' }}>
      <div className="w-full h-fit text-center font-bold text-[50px]">Execoms</div>
      <div className="w-full mt-5 ml-8 text-[25px] flex gap-3">
        <button onClick={handlePrevious} disabled={noPrevious} className={`rounded-[100%] h-10 w-10 flex justify-center items-center transition-all duration-500 ${noPrevious ? "opacity-10" : "hover:bg-slate-300 hover:scale-105 "}`}>
          <GrLinkPrevious className='hover:scale-105' />
        </button>
        <p className='transition-all duration-1000'>{activeYear}</p>
        <button onClick={handleNext} disabled={noNext} className={` rounded-[100%] h-10 w-10 flex justify-center items-center transition-all duration-500 ${noNext ? "opacity-10" : "hover:bg-slate-300 hover:scale-105 "}`}>
          <GrLinkNext />
        </button>
      </div>
      <div className="flex flex-wrap gap-4 mt-7 justify-center">
        {loading ? (
          ...
          TeamData && TeamData.map(item => (
            <div key={item.name} className="w-56 mx-7 mt-14 mb-14 h-[230px] bg-white border-gray-300 rounded-2xl shadow-xl  transition-all duration-100" data-aos={slidemove}>
